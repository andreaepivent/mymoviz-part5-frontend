...
function Home() {
  ...
  // Fonction pour formater l'overview
  function formatOverview(overview) {
    if (overview.length > 250) {
      return overview.substring(0, 247) + "...";
    }
    return overview;
  }

  useEffect(() => {
    fetch("https://mymoviz-part5-backend-rose.vercel.app/movies")
      ...
      .then((data) => {
        const formattedMovies = data.movies.map((movie) => ({
          ...
          overview: formatOverview(movie.overview),
        }));
  ...
  const movieList = movies.map((data, i) => {
  ...
  return (
    <div className={styles.main}>
      ...
      <div className={styles.moviesContainer}>{movieList}</div>
    </div>
  );
}
